...
There are multiple sources highlighting the low success rates of large IT-Enabled projects[^9]. This because we used to quantify “success” as being on time, on budget, and as per requirements. There has been significant progress these last few years in quantifying success towards "what this investment means to canadians" (especially coming from our [pandemic response successes](https://nationalpost.com/opinion/john-ivison-amid-staggering-unemployment-rate-public-servants-processing-ei-claims-are-the-unsung-heroes)). Authorities are then now seeking assurances that a particular investment adds value. So then, how do we quantify value? And how do we give confidence to those authorities that we are not asking for a blank cheque?
